...
export class PersistentCacheManager {
  ...
  public constructor(trackCacheMetrics: boolean){
    this.trackCacheMetrics = trackCacheMetrics;
  }
  ...
  /**
   * Starts a session tracking performance characteristics of 
   * the cache underlying a key.
   * @param cacheKey
   * @returns
   */
  public startTrackingSession<I, S, C>(cacheKey: ICacheKey<I, S, C>): ICacheMetrics{
    const cache = this.ensureCache(cacheKey);
    const statistics = cache.cache.startSession();
    return statistics
  }

  /**
   * Ends the session and returns the statistics
   * @param cacheKey 
   * @returns 
   */
  public endTrackingSession<I, S, C>(cacheKey: ICacheKey<I, S, C>): ICacheMetrics {
    const cache = this.ensureCache(cacheKey);
    const statistics = cache.cache.endSession();
    return statistics
  }
  
  public endAllSessions(){
    const cacheStats: Record<string, ICacheMetrics> = {};
    for (const [key, cache] of this.cacheRegistry.entries()){
      cacheStats[key] = cache.cache.endSession();
    }
    return cacheStats;
  }
