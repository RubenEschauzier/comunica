import type { IAggregateValues, IQueryGraphViews, IResultSetRepresentationGraph } from '@comunica/mediator-join-reinforcement-learning';
...
export class ExperienceBuffer {
  ...
  public getRandomExperience(): [IExperience, IExperienceKey] {
    const index = Math.trunc(Math.random() * this.getSize());
  ...
  public setExperience(queryKey: string, joinPlanKey: string, experience: IExperience, runningMomentsY: IAggregateValues) {
    ...
    this.experienceAgeTracker.push({ query: queryKey, joinPlanKey });
  ...
  public queryExists(queryString: string) {
    return Boolean(this.queryToKey.get(queryString));
  }
  ...
  public loadLeafFeatures(fileLocation: string) {
    ...
    for (const entry of data) {
      ...
      const featureGraphViews: IQueryGraphViews = { subSubView: features[2][0],
        objObjView: features[2][1],
        objSubView: features[2][2] };

      newLeafFeatures.set(key, { hiddenStates: featureTensorHS,
        memoryCell: featureTensorMem,
  ...
  public loadInitTracking(fileLocationMap: string, fileLocationUnique: string) {
    ...
    for (const element of queryToKeyMapArray) {
      loadedQueryToKeyMap.set(element[0], element[1]);
    }
