...
export class ActorRdfJoinInnerMultiReinforcementLearning extends ActorRdfJoin {
  ...
  protected async getJoinCoefficients(
    ...
    */
    ...
    if (this.offlineTrain){
      ...
      const choiceToAdd: MCTSJoinInformation = {N: prevPrediction.N, meanValueJoin: valueProbabilityJoin[0], 
        totalValueJoin: prevPrediction.totalValueJoin, priorProbability: valueProbabilityJoin[1],featureMatrix: featureMatrix, adjencyMatrix: adjacencyMatrixCopy};
  ...
  public async MCTSOffline(action: IActionRdfJoin, modelHolder: modelHolder, masterTree: MCTSMasterTree){
    ...
    for(let i=0; i<numSimulations; i++){
      ...
      await this.MCTSearchTEST(cloneAction, modelHolder, masterTree, state.getJoinIndexes());
    ...
    for (const join of possibleChildren){
      ...
      const improvedPolicyProbability = masterTree.getJoinInformation(joinKey).N / rootInformation.N;
    ...
    const adjMatrixCopy = action.context.getEpisodeState().adjacencyMatrix;
    adjMatrixCopy[adjMatrixCopy.length-1][adjMatrixCopy[0].length-1] = 1;
    return [estimatedValuesChildren[idx], improvedProbabilitiesChildren[idx]];
  }
  
  public async MCTSearchTEST(action: IActionRdfJoin, modelHolder: modelHolder, MCTSMasterTree: MCTSMasterTree, rootJoin: number[][]){
    /**
    ...
    if (currentJoinStateInformation.N>0){
      ...
      if (possibleJoins.length == 0){
        ...
        const outputChild = await this.getOutputChildTest(clonedAction, modelHolder, MCTSMasterTree, currentJoinStateKey);  
      ...
      await this.MCTSearchTEST(clonedAction, modelHolder, MCTSMasterTree, rootJoin);
      return;
    }
    else{
      // 3. If we haven't, expand the child states and add them to MCTS
      await this.expandChildStatesTest(clonedAction, modelHolder, MCTSMasterTree);
  ...
  private async getOutputChildTest(action: IActionRdfJoin, modelHolder: modelHolder, masterTree: MCTSMasterTree, joinKey: number[][]): Promise<number[]>{
    /*
    1. clone action
  ...
  private async expandChildStatesTest(action: IActionRdfJoin, modelHolder: modelHolder, masterTree: MCTSMasterTree){
    ...
    for (const join of possibleChildren){
      ...
      const predictionOutput = await this.getOutputChildTest(action, modelHolder, masterTree, addedJoinKey);
      priorProbabilitiesEstimated.push(predictionOutput[1]);
      // console.log("Expanded Child:");
      // console.log(addedJoinKey);
    }
