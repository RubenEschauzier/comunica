...
import { eddiesContextKeys, IEddieBindingsMetadata, ITimestampGenerator } from './EddieControllerStream';
...
export class EddieOperatorStream extends BufferedIterator<Bindings>{
  ...
  private sourceIterator: BindingsStream;

  private tripleMap: Map<number, Bindings[]> = new Map();
  private timestampGenerator: ITimestampGenerator;

  private funHash: HashFunction;
  private funJoin: JoinFunction;

  private readBuffer: boolean = true; 
  private bitLoc: number;

  private match: Bindings | null = null;
  private matches: Bindings[] = [];
  private matchIdx: number = 0;

  private tempTotalReads: number = 0;
  ...
  override read() {
    while(true){
      ...
      // TODO Allow joins on all variables that this stem represents!
      ...
      if (!joinVars){
        ...
        for (const joinVar of this.joinVariables){
          ...
          let result = this.tripleMap.get(hash)!;
          result.push(item);
        }
